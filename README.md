...
以下步骤将以阿里云服务器（ECS）为例，指导如何将Neko-Dictation 项目部署到阿里云服务器（ECS），并且购买域名，将其绑定到服务器的 IP 地址。不太想写。
...
1. 通过 SSH 登录阿里云服务器，使用 `wget` 下载并解压你的打包程序。
2. 运行打包的可执行文件，确保设置了执行权限。
3. 配置阿里云域名，解析到服务器的 IP 地址。
4. 确保阿里云安全组规则允许访问你的应用端口。
5. 通过服务器 IP 或域名访问应用。
